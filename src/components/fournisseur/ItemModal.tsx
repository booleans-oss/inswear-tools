import type { FournisseurItem, Size } from "@/utils/types";
...
import { ItemSchema } from "@/schemas/FournisseurItem";
import SizeRadio from "../form/SizeRadio";

type ItemModalProps = {
  item: FournisseurItem | undefined;
  onClose: () => void;
  isOpen: boolean;
  margin: number;
  onSave: (item: FournisseurItem) => void;
  onDelete: () => void;
};
...
const ItemModal: FC<ItemModalProps> = ({
  ...
  const [color, url, size, customizations, quantity] = watch([
    "color",
    "url",
    "size",
    "customizations",
  ...
  const onSubmit: SubmitHandler<FournisseurItem> = (data) => {
    onSave(data);
    onClose();
  };
